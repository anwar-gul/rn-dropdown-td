...
function Picker ({
  ...
  const _listMessageTextStyle = useMemo(
    () => [THEME.listMessageText, ...[listMessageTextStyle].flat()],
    ...
    (item, customItem = false) => {
      ...
      setNecessaryItems(state => {
        if (multiple) {
          ...
          if (
            _state.findIndex(x => x[_schema.value] === item[_schema.value]) > -1
          ) {
            // Remove the item
            if (Number.isInteger(min) && min >= _state.length) {
              return state
  ...
  /**
   * The clear icon component.
   * @returns {JSX.Element}
   */
  const _ClearIconComponent = useMemo(() => {
    if (listMode !== LIST_MODE.MODAL) return null

    let Component

    if (ClearIconComponent !== null)
      Component = <ClearIconComponent style={_closeIconStyle} />
    else Component = <Image source={ICON.CLOSE} style={_closeIconStyle} />

    return (
      <TouchableOpacity style={_closeIconContainerStyle} onPress={onPressClear}>
        {Component}
      </TouchableOpacity>
    )
  }, [
   ...
   * @returns {JSX.Element}
   */
  const _BackIconComponent = useMemo(() => {
    if (listMode !== LIST_MODE.MODAL) return null

    let Component

    if (BackIconComponent !== null)
      Component = <BackIconComponent style={_closeIconStyle} />
    else Component = <Image source={ICON.CLOSE} style={_closeIconStyle} />
